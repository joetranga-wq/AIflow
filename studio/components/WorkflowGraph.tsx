...
interface WorkflowGraphProps {
  ...
  // 🆕 Validation badges per agent
  validationByAgentId?: Record<string, ValidationSummary>;
...
const WorkflowGraph: React.FC<WorkflowGraphProps> = ({ 
  ...
  // Zoom voor auto-center op geselecteerde node
  ...
  // --- Initial render / graph construction ---
  useEffect(() => {
    ...
    const simulation = d3
      ...
      .force(
        'link',
        d3.forceLink(validLinks).id((d: any) => d.id).distance(250),
      )
    ...
    // Klik op lijn → selecteer edge
    linkGroup.on('click', (event: any, d: any) => {
    ...
    const labelGroup = g
      ...
      .data(validLinks)
      .join('g')
      .attr('class', 'link-label-group cursor-pointer hover:opacity-80 transition-opacity')
    ...
    const linkLabelRect = labelGroup
      .append('rect')
    ...
    agentNodes
      ...
      .attr('rx', 4)
      .attr('fill', (d: any) =>
        d.id === project.flow.entry_agent ? '#10B981' : '#6366F1',
      )
    ...
    agentNodes
      .append('text')
      .text((d: any) =>
        d.modelName ? d.modelName.substring(0, 12) + '...' : 'model',
      )
      .attr('x', -45)
      .attr('y', 29)
      .attr('text-anchor', 'middle')
    ...
    // --- Validation badges (errors/warnings per node) ---
    const validationGroup = agentNodes
      .append('g')
      .attr('class', 'validation-badge')
      .attr('transform', 'translate(-92,-34)');
    ...
    // --- Hover Buttons ---
    const foreignObject = agentNodes
    ...
    const actionContainer = foreignObject
      .append('xhtml:div')
      .style('display', 'flex')
      .style('gap', '4px')
      .style('opacity', '0')
    ...
    createHoverBtn(
      ...
      'Edit Prompt',
      (d) => onNavigateToPrompt && onNavigateToPrompt(d.promptFile),
    );
    createHoverBtn(
      ...
      'Configure',
      (d) => onSelectAgent(d.id),
    );

    agentNodes
      .on('mouseenter', function (this: any) {
        d3.select(this).select('.hover-actions').style('opacity', '1');
      })
      .on('mouseleave', function (this: any) {
        d3.select(this).select('.hover-actions').style('opacity', '0');
      });
    ...
    simulation.on('tick', () => {
      linkGroup.attr(
        'd',
        (d: any) =>
          `M${(d.source as any).x},${(d.source as any).y} L${(d.target as any).x},${(d.target as any).y}`,
      );
  ...
  useEffect(() => {
    ...
    svg
      ...
      .attr('stroke', (d: any) => {
        const isOnPath =
          Array.isArray(highlightedNodeIds) && highlightedNodeIds.includes(d.id);
    ...
    // Links: highlight either full path or local selection / linking / selected edge
    svg
      .selectAll('.link-path')
      .attr('stroke', (d: any) => {
        ...
        const isInPath = hasPath
          ? highlightedEdges!.some((e) => e.from === sourceId && e.to === targetId)
          : false;
      ...
      .attr('opacity', (d: any) => {
        ...
        if (!selectedNodeId && !isLinkingMode && !hasPath && !selectedLinkId) {
          return 1;
        }
    ...
    // Link labels mee-stylen met geselecteerde edge
    svg
      .selectAll('.link-label-rect')
      .attr('fill', (d: any) => {
        const isSelected = selectedLinkId && d.id === selectedLinkId;
        return isSelected ? '#EEF2FF' : '#F8FAFC';
      })
      .attr('stroke', (d: any) => {
        const isSelected = selectedLinkId && d.id === selectedLinkId;
        return isSelected ? '#6366F1' : '#E2E8F0';
      })
      .attr('stroke-width', (d: any) => {
        const isSelected = selectedLinkId && d.id === selectedLinkId;
    ...
    svg
      ...
      .attr('fill', (d: any) => {
        const isSelected = selectedLinkId && d.id === selectedLinkId;
        return isSelected ? '#4F46E5' : '#64748B';
      });

        // Validation badges updaten op basis van validationByAgentId
        svg.selectAll('.validation-badge').each(function (d: any) {
          const agentId = d.id;
          const summary: ValidationSummary | undefined = validationByAgentId
            ? validationByAgentId[agentId]
            : undefined;
          ...
          g.style('display', hasIssues ? 'block' : 'none');

          // ✅ hier definiëren we de variabelen weer
          const circle = g.select('circle.validation-circle');
          const textEl = g.select('text.validation-text');
          ...
          if (errors > 0) {
            ...
            textEl
              .attr('fill', '#B45309')
              .text(warnings > 9 ? '9+' : String(warnings));
          }
        });
  ...
  // --- Auto-zoom + kleine flash op geselecteerde node ---
  useEffect(() => {
    ...
    const scale = 1.25;
    const x = clientWidth / 2 - node.x * scale;
    const y = clientHeight / 2 - node.y * scale;
    const transform = d3.zoomIdentity.translate(x, y).scale(scale);

    svg
      .transition()
      .duration(450)
      .call(zoomRef.current.transform, transform as any);

    // Kleine flash op de geselecteerde node
    svg
      .selectAll('.node-rect')
    ...
    >
      ...
      {contextMenu && (
        ...
        >
          ...
          <button
            onClick={() => {
              onSelectAgent(contextMenu.agentId);
              setContextMenu(null);
            }}
          ...
          {!contextMenu.isTool && (
            <>
              <button
                onClick={() => {
                  if (contextMenu.promptFile && onNavigateToPrompt)
                    onNavigateToPrompt(contextMenu.promptFile);
                  setContextMenu(null);
                }}
                className="w-full text-left px-4 py-2 text-sm text-slate-700 hover:bg-slate-50 hover:text-indigo-600 transition-colors"
              >
